...
pub(super) fn dispatch_hub_action(app: &mut App, action: HubAction) -> Result<()> {
    match action {
        ...
        HubAction::CopyReviewJson => {
            app.copy_review_json()?;
        }
        HubAction::CopyQuestionsJson => {
            app.copy_questions_json()?;
        }
    ...
    match key.code {
        KeyCode::Char('y') => {
            ...
            } else if let InputMode::Confirm(ConfirmAction::DeleteWatchedFile { ref path }) = action
            {
                ...
                match er_engine::github::gh_pr_approve(&repo_root, remote.as_deref(), pr) {
...
pub(super) fn build_agent_questions_prompt(app: &mut App) -> Option<String> {
    ...
    if tab.is_remote() {
        ...
        let output_dir = app.tab().er_dir();
        return Some(er_engine::ai::prompts::build_questions_prompt_remote(
            parts[0],
            parts[1],
            pr_number,
            &output_dir,
...
pub(super) fn build_agent_summary_prompt(app: &mut App) -> Option<String> {
    ...
    if tab.is_remote() {
        ...
        let output_dir = app.tab().er_dir();
        return Some(er_engine::ai::prompts::build_summary_prompt_remote(
            parts[0],
            parts[1],
            pr_number,
            &output_dir,
...
pub(super) fn sync_github_comments(app: &mut App) -> Result<()> {
    ...
    let mut gc: er_engine::ai::ErGitHubComments = match std::fs::read_to_string(&comments_path) {
        Ok(content) => {
            serde_json::from_str(&content).unwrap_or_else(|_| er_engine::ai::ErGitHubComments {
                version: 1,
    ...
    gc.github = Some(er_engine::ai::GitHubSyncState {
