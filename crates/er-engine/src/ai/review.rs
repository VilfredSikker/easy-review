...
/// Confidence in a finding, set by the AI based on what it could verify.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, Default, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum Confidence {
    /// Default for older review.json files. Review pass uses this when a finding
    /// couldn't be verified within the read budget. Validate pass should resolve.
    #[default]
    Tentative,
    /// Evidence supports the finding; the reviewer should act on it.
    Confirmed,
    /// Real, but pre-existing or low-impact relative to this PR. Optional fix.
    Informational,
    /// Validate disproved the concern. Kept for audit trail; rendered dimmed.
    Dropped,
}
...
pub struct Finding {
    ...
    /// True when the AI flagged the referenced code as outside the diff hunks
    /// (i.e. the issue was not introduced by this PR). Downstream agents reading
    /// this file should treat such findings as informational — fix is optional
    /// and may belong in a separate change.
    #[serde(default)]
    pub outside_diff: bool,
    /// AI's confidence in this finding. Set by review (mostly confirmed/informational/tentative)
    /// and refined by validate (confirmed/informational/dropped).
    #[serde(default)]
    pub confidence: Confidence,
    /// One-line description of what would resolve a Tentative finding —
    /// what file/symbol/test the validate pass should read or grep.
    #[serde(default)]
    ...
    /// when the issue is no longer present in the current diff/file. Orthogonal
    /// to `confidence` — a Confirmed finding can become resolved.
    #[serde(default)]
    pub resolved: bool,
    /// One-line summary of what changed that resolved the finding (e.g.
    /// "line 42 now has the null check", "function deleted").
    #[serde(default)]
...
// ── Comment index for O(1) lookups ──

/// Identifies which source collection a comment came from
#[derive(Clone, Debug)]
enum CommentSource {
    Question,
    Note,
...
/// Built lazily on first query, invalidated when questions/github_comments change.
struct CommentIndexData {
    /// (file, hunk_index) -> vec of (source, index_in_source_vec)
    hunk_index: HunkIndexMap,
...
impl AiState {
    ...
    fn ensure_index(&self) {
        ...
        if let Some(qs) = &self.questions {
            for (i, q) in qs.questions.iter().enumerate() {
                // Hunk index: all questions (including replies, matching comments_for_hunk behavior)
                hunk_index
                    .entry((q.file.clone(), q.hunk_index))
                    .or_default()
                    .push((CommentSource::Question, i));
                // Line index: top-level only (matching comments_for_line behavior)
                if let Some(ls) = q.line_start {
                    if q.in_reply_to.is_none() {
                        line_index
                            .entry((q.file.clone(), ls))
                            .or_default()
                            .push((CommentSource::Question, i));
                    }
                }
        ...
        if let Some(gc) = &self.github_comments {
            for (i, c) in gc.comments.iter().enumerate() {
                // Hunk index: all comments (including replies, matching comments_for_hunk behavior)
                hunk_index
                    .entry((c.file.clone(), c.hunk_index))
                    .or_default()
    ...
    pub fn comments_for_hunk_or_line_range(
        ...
        let comment_matches_hunk = |c: &CommentRef<'_>| -> bool {
            let ls = c.line_start();
            let ols = c.old_line_start();
            if ls.is_none() && ols.is_none() {
                return c.hunk_index() == Some(hunk_idx);
            }
        ...
        if let Some(qs) = &self.questions {
            for q in &qs.questions {
                if q.file == path && q.in_reply_to.is_none() {
                    ...
                    if comment_matches_hunk(&c) && seen_ids.insert(q.id.clone()) {
                        result.push(CommentRef::Question(q));
                    }
                }
            }
        ...
        if let Some(ns) = &self.notes {
            for n in &ns.notes {
                if n.file == path && n.in_reply_to.is_none() {
                    ...
                    if comment_matches_hunk(&c) && seen_ids.insert(n.id.clone()) {
                        result.push(CommentRef::Note(n));
                    }
                }
            }
        ...
        if let Some(gc) = &self.github_comments {
            for c in &gc.comments {
                if c.file == path && c.in_reply_to.is_none() {
                    ...
                    if comment_matches_hunk(&cr) && seen_ids.insert(c.id.clone()) {
                        result.push(CommentRef::GitHubComment(c));
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn comments_for_hunk_or_line_range_left_comment_matches_old_range_not_earlier_new_line() {
        // LEFT comments store the old-side number in line_start. After a large
        // insertion that number can also appear as a new-side line in an earlier
        // hunk. The comment belongs on the hunk that contains the deleted line.
        let mut state = AiState::default();
        let mut c = make_github_comment("c1", "pipeline.py", Some(1), None);
        c.line_start = Some(222);
        c.old_line_start = Some(222);
        c.side = "LEFT".to_string();
        state.github_comments = Some(ErGitHubComments {
            version: 1,
            diff_hash: "test".to_string(),
            github: None,
        ...
        assert!(
            hunk0.is_empty(),
            "LEFT line 222 must not attach to an earlier hunk just because new-side 222 exists there"
        );

        let hunk1 = state.comments_for_hunk_or_line_range("pipeline.py", 1, 250, 26, 200, 50);
        assert_eq!(hunk1.len(), 1);
        assert_eq!(hunk1[0].id(), "c1");
    }

    #[test]
    fn comments_for_hunk_or_line_range_left_question_matches_old_range_not_earlier_new_line() {
        let mut state = AiState::default();
        let mut q = make_question("q1", "pipeline.py", Some(1));
        q.line_start = Some(222);
        q.old_line_start = Some(222);
        q.side = "LEFT".to_string();
        state.questions = Some(ErQuestions {
            version: 1,
            diff_hash: "test".to_string(),
        ...
        let hunk0 = state.comments_for_hunk_or_line_range("pipeline.py", 0, 218, 20, 140, 20);
        assert!(hunk0.is_empty());
        let hunk1 = state.comments_for_hunk_or_line_range("pipeline.py", 1, 250, 26, 200, 50);
        assert_eq!(hunk1.len(), 1);
        assert_eq!(hunk1[0].id(), "q1");
    }

    #[test]
    fn comments_for_hunk_or_line_range_left_comment_matches_mixed_delete_hunk() {
        let mut state = AiState::default();
        let mut c = make_github_comment("c1", "a.rs", Some(0), None);
        c.line_start = Some(10);
        c.old_line_start = Some(10);
        c.side = "LEFT".to_string();
        state.github_comments = Some(ErGitHubComments {
            version: 1,
            diff_hash: "test".to_string(),
        ...
        // Mixed hunk: deletes and adds. new_count > 0, old range still owns the LEFT line.
    ...
    // ── Confidence + Finding backwards compatibility ──

    #[test]
    fn finding_deserialises_without_new_fields_to_tentative() {
        // An older review.json predating confidence/evidence should still load,
        // and missing `confidence` should default to Tentative.
        let json = r#"{
            "id": "f-old",
            "severity": "medium",
            "title": "Old finding",
        ...
        let f: Finding = serde_json::from_str(json).expect("legacy finding should deserialise");
        assert_eq!(f.id, "f-old");
        assert_eq!(f.confidence, Confidence::Tentative);
        assert!(f.evidence.is_empty());
        assert_eq!(f.verification_plan, "");
    }
    ...
    fn confidence_serialises_lowercase() {
        ...
        let json = serde_json::to_string(&Confidence::Informational).unwrap();
        assert_eq!(json, "\"informational\"");
        let json = serde_json::to_string(&Confidence::Tentative).unwrap();
        assert_eq!(json, "\"tentative\"");
        let json = serde_json::to_string(&Confidence::Dropped).unwrap();
