...
Specialized **Patterns** review for the current git diff. Writes `.er/experts/patterns.json` only.
...
`/er-review-patterns` or `/er-review-patterns [scope] [base-branch]`
...
Focus: Consistency with existing code.

- `category`: `patterns`
- Finding ids: `pat-1`, `pat-2`, …
- Caps: **2 per file, 10 total**
- Target under 2 minutes

## Output

`mkdir -p .er/experts` then write **only** `.er/experts/patterns.json`:
