...
2. Reads `.er-review.json`, `.er-summary.md`, `.er-checklist.json`, and `.er-feedback.json` from the repo root
...
TOOL CALLS 2-5 — Read all review artifacts (parallel):
  ...
  Read .er-feedback.json  (optional — include human comments if diff_hash matches)
...
IN-CONTEXT (zero tool calls) — Build the complete payload JSON:
  ...
  1. Build the review body markdown string:
     ...
     ### Human Comments
     <From .er-feedback.json if fresh>
