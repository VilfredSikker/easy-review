...
- `max_concurrent_reviews` bounds the background review queue and arena reviewer rounds (default 3; the pickers offer 1–6 in the TUI and 1–16 in the desktop). Only the background review dispatch and the arena reviewer rounds acquire a slot; `spawn_agent_prompt`, `card_ai_spawn`, `spawn_command` and `model_discovery::run_models_command` spawn without one. See `docs/adr/0021-agent-concurrency.md`.
