...
/** Outcome-shaped checklist: the categories the generator prompt asks for, one
 *  per group, with a related file and finding for the card's jump links. */
export const checklistFixture: ChecklistSnapshot = {
  fresh: true,
  items: [
    {
      id: "c-1",
      text: "The migration is reversible and no backfill runs inside the write lock.",
      category: "schema",
      checked: true,
      related_findings: [],
      related_files: ["packages/discovery-platform/src/lib/variant-warning-copy.ts"],
    },
    {
      id: "c-2",
      text: "Tests cover a missing severity, not only the happy-path variant.",
      category: "tests",
      checked: false,
    ...
    {
      id: "c-3",
      text: "The public helper keeps its signature; the new argument is optional.",
      category: "api",
      checked: false,
      related_findings: ["finding-2"],
      related_files: ["packages/discovery-platform/src/lib/experiment-template-resolution.ts"],
    },
    {
      id: "c-4",
      text: "No auth or authz path in this diff, so nothing to re-check here.",
      category: "auth",
      checked: true,
      related_findings: [],
      related_files: [],
    ...
    {
      id: "c-5",
      text: "Matches the ask: warnings become copy templates, not new pages.",
      category: "plan",
      checked: false,
      related_findings: [],
      related_files: [],
...
export const aiWithFindings: AiSnapshot = {
  ...
  resolved_findings: [],
  dropped_findings: [],
  min_trust_default: "informational",
...
export const aiProfessorOnly: AiSnapshot = {
  ...
  eligible_comment_count: 0,
  triage: null,
  // A professor pass writes no checklist — only the general review does.
  checklist: null,
  diagrams: [],
  diagram_presets: diagramPresetsFixture,
};
