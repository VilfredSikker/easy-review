...
<script lang="ts">
  ...
  async function runReview() {
    if (!reviewScope || selectedCount === 0 || submitting) return;
    const scope = reviewScope;
    const paths = [...selected];
    submitting = true;
    try {
      close();
      await app.cmd("run_ai_review_files", { scope, paths });
    } finally {
      submitting = false;
    }
  }
...
>
  <div class="px-4 pt-3 pb-2 border-b border-ink-600 flex items-center gap-2 shrink-0">
    <span class="text-xs text-ink-300 font-mono">Review select files</span>
    <span class="text-[10px] text-ink-400 font-mono ml-1">
      {scopeLabel}{#if pickerFiles.length > 0} · {pickerFiles.length} files{/if}
    </span>
  ...
  <div class="px-4 py-3 border-t border-ink-600 flex items-center justify-end gap-2 shrink-0">
    ...
    <Button
      variant="primary"
      disabled={loading || !!loadError || selectedCount === 0 || submitting}
