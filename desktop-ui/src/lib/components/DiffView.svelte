<script lang="ts">
  ...
  let composerState = $state<ComposerState | null>(null);

  function openComposer(hunk_idx: number, line_num: number | null) {
    if (!selectedFile) return;
    composerState = { file: selectedFile.path, hunk_idx, line_num };
  }
...
<div class="flex-1 flex flex-col min-w-0 overflow-hidden">
  ...
  <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
    <div class="flex-1 overflow-y-auto font-mono text-[13px] leading-[1.55]">
      ...
      {:else}
        {#each selectedFile.hunks as hunk, hunkIndex}
          <div class="px-4 py-1 text-ink-300 bg-ink-850 border-b border-ink-500/40 text-[12px]">
            {hunk.header}
          </div>
          {#each hunk.lines as line}
            <div
              class="grid grid-cols-[40px_40px_1fr] cursor-pointer {lineClass(line.kind)} {line.kind === 'context' ? 'hover:bg-ink-800/50' : ''}"
              onclick={() => {
                if (line.kind !== "fold") {
                  openComposer(hunkIndex, line.new_num ?? line.old_num ?? null);
                }
              }}
            >
          ...
          {/each}
          <!-- Inline threads for this hunk -->
          {#each hunk.threads as thread}
    ...
    <!-- Composer: sits below scroll area, resets when composerState key changes -->
    {#if composerState !== null}
      {#key composerState}
        <DiffComposer
          file={composerState.file}
          hunk_idx={composerState.hunk_idx}
          line_num={composerState.line_num}
