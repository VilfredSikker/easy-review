...
- **AI review queue + global agent cap (desktop).** Review/expert/professor/triage spawns go through `spawn_background_agent_task` which enqueues into `App.pending_background_tasks` (FIFO) when the running count reaches `ai_hub.max_concurrent_reviews` (default 3; the desktop accepts 1–16, the TUI config hub offers 1–6). `poll_background_tasks` dispatches queued tasks as slots free up; queued tasks render as a "queued" pill (cancellable via `cancel_queued_review`). Independently, `er_engine::agent_slots` is a process-wide counting semaphore. Background reviews and arena reviewer rounds share it, so N simultaneous arena runs can't multiply the process count. It does **not** cover every spawn: the tab-level `spawn_agent_prompt`, card AI, the configured shell command (`spawn_command`), and the arena arbiter all spawn without taking a slot or entering the queue — see `docs/plans/plan-agent-runner-and-cpu-audit.md` for the six spawn sites and their coverage.
