...
- **`app/`** — All application state in one `App` struct. Three diff modes (Branch, Unstaged, Staged). Five input modes (Normal, Search, Comment, Confirm, Filter). File/hunk/line navigation, AI state management, comment persistence (with replies and deletion), comment focus navigation, watch notifications. Composable filter system (`filter.rs`) with glob, status, and size rules.
...
src/app/state.rs         App struct, all state, navigation, comments, comment focus, replies, filter
