...
- **Client-side syntax highlighting (desktop).** `build_hunks` sends plain `LineSnapshot.text` only (no spans over IPC). Shiki runs in a Web Worker (`highlightWorker.ts`); `FlatDiffView.svelte` fills `spans` client-side with LRU cache (`highlightCache.ts`, 50 files, keyed by `filePath::cacheKey::theme`). Up to 4 concurrent highlight requests; viewport-visible files only. TUI still uses syntect in Rust.
- **Shared theme (TUI + desktop).** `display.theme` drives both UIs. The desktop ports the 8 TUI palettes (`desktop-ui/src/lib/themes.ts`, mirrors `er-tui/src/ui/themes.rs`) and applies them by overriding the `app.css` CSS custom properties on the document root — the `--color-ink-*` ladder is interpolated from each theme's bg/surface/panel/border/text anchors. Each theme maps to a bundled Shiki theme (`syntaxThemes.ts`: one-dark-pro, one-light, tokyo-night, github-dark-high-contrast) and themes the embedded xterm terminal. Snapshots carry `theme`, so settings changes restyle the app live; the Theme picker sits in the General settings tab.
...
- **Heavy Tauri commands run off the main thread (desktop).** Sync Tauri commands execute on the main thread; commands that wait on the `App` mutex or shell out to git/gh (`poll`, `select_tab`, `set_mode`, `get_snapshot`, `select_file`, `request_file_content`, `open_local_branch`, `open_pr_review`, `open_pr_branch`, `open_pr_url`, `open_remote_pr`, `open_inbox_item`, `set_filter`, `clear_filter`, `pull_github_comments`, `push_github_comments`, `push_github_comment_thread`, `push_github_comment_reply`, `submit_github_review`, `resync_github_status`) are `async fn`s that wrap their blocking body in `run_blocking` (`tauri::async_runtime::spawn_blocking`). `AppState` is all `Arc`s and derives `Clone` for this. New heavy commands should follow the same pattern — never block the main thread on the `App` lock. The comment pull/push commands additionally run three-phase (brief lock to capture identity → `gh` network without the lock → brief lock to apply) via the pure cores in `er_engine::sync`, so they never starve the poll loop either.
- **PR opens never block on `gh`/`git fetch` when a cached diff exists (desktop).** A disk/cache-hit `open_pr_review` skips the synchronous `gh pr commits` call (`commits_pending` → `spawn_local_pr_commits_backfill` applies them under a brief lock + revision bump) and resolves the base ref via `resolve_base_ref_nonblocking`: TTL-cached (`er_engine::github::BaseRefCache`, 60s) → local `git rev-parse` → best-effort `origin/<base>` name with a deduplicated background `git fetch`. Target profile on a disk hit: `invoke_ms` well under 150.
- **Persistent branch-diff cache (desktop).** Committed-state branch diffs (`git diff base...branch`) are cached in `er_engine::diff_store` under `repos/<slug>/branch-diffs/<branch>/` keyed by `(head oid, base oid)`. `open_local_branch` serves an exact-key hit instantly, serves a stale entry with SWR (`loading.remote_pr_diff` → "Updating…" pill; the background base-fetch refresh swaps in the fresh diff), and otherwise defers the first `git diff` to a worker (`loading.tab_diff` → "Loading diff…"). Every successful branch refresh (deferred, background, watch-independent) writes through via `persist_branch_diff`.
