...
- **Per-view artifact scoping (Local Branch vs PR Diff).** A local PR tab reviews two different diffs — the local branch (Branch/Unstaged/Staged/History views) and the PR head-vs-base (PR Diff view) — so most artifacts are *split* per view bucket: triage, `review.json`, professor/experts, `questions.json`/`notes.json`, `reviewed`, and `checklist.json` (`apply_managed_root` routes only the `Pr` bucket to `prs/pr-<N>/`; everything else falls through to `branches/<b>/view-buckets/<bucket>/`). Two artifacts are *shared* instead: **GitHub PR comments** are PR-scoped (they belong to the PR and two-way sync with GitHub), so `github_comments_dir()` always resolves to the PR bucket and `reload_ai_state` overrides them from there in every view; and the **guided tour** walks the branch diff, so it lives in the branch view bucket (`branch_bucket_er_dir`) and is shown across the working-tree views but cleared in the PR Diff view. Promote-to-comment is the deliberate one-way bridge from a private question/note into the shared PR space.
