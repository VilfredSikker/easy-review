...
Workspace layout: `crates/er-engine/` (UI-agnostic core, `er_engine` crate), `crates/er-tui/` (terminal UI, `er` binary), `crates/er-desktop/` (Tauri backend, `er-desktop` binary), and `desktop-ui/` (Svelte frontend, built by Vite, bundled by Tauri).
