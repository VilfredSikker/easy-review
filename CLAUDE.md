...
- **`app/`** — All application state in one `App` struct. Three diff modes (Branch, Unstaged, Staged). Three input modes (Normal, Search, Comment). File/hunk/line navigation, AI state management, comment persistence, watch notifications. Loads `.er-config.toml` for watched files configuration.
...
src/app/state.rs         App struct, all state, navigation, comments, watched files config
...
**v1.1 (current):** AI review integration (4 view modes, inline findings, comments), GitHub PR support (`--pr` flag, URL arguments), line-level navigation (arrow keys), comment system (`c` key → `.er-feedback.json`), watched files for git-ignored paths (`.er-config.toml`).
