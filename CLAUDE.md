...
- **Per-view artifact scoping (Local Branch vs PR Diff).** A local PR tab reviews two different diffs — the local branch (Branch/Unstaged/Staged/History views) and the PR head-vs-base (PR Diff view) — so most artifacts are *split* per view bucket: triage, `review.json`, professor/experts, `questions.json`/`notes.json`, `reviewed`, and `checklist.json` (`apply_managed_root` routes only the `Pr` bucket to `prs/pr-<N>/`; everything else falls through to `branches/<b>/view-buckets/<bucket>/`). The two exceptions:
  ...
  - **Guided tour** is split per view too, but with cross-bucket reuse (`resolve_view_tour`): the Local branch tour lives in the branch bucket (shown across the working-tree views), the PR Diff tour lives in the PR bucket, and a tour whose `diff_hash` matches the active diff is reused across the two — so when the branch and PR diffs are identical one generation serves both, and once they drift each view keeps its own. `generate_tour` writes to the active view's bucket; `build_tour_snapshot` sets `fresh = tour.diff_hash == branch_diff_hash`, and the desktop Guide header shows a stale pill + Regenerate button when it drifts. The Guide tab is hidden in the PR Diff view only when that view has no (own or reusable) tour.
