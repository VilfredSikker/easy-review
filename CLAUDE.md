...
- **`app/`** — All application state in one `App` struct. Four diff modes (Branch, Unstaged, Staged, History). Four input modes (Normal, Search, Comment, Filter). File/hunk/line navigation, AI state management, comment persistence, watch notifications. History mode has its own `HistoryState` with commit list, per-commit diff cache (LRU, 5 entries), and lazy loading. Loads `.er-config.toml` for watched files configuration. Composable filter system (`filter.rs`) with glob, status, and size rules. Mtime sort toggle (`Shift+R`) works in any diff mode.
...
src/app/state.rs         App struct, all state, navigation, comments, HistoryState, DiffCache, watched files config, filter
...
src/git/status.rs        detect_base_branch(), git_diff_raw(), staging, worktrees, git_log_branch(), git_diff_commit(), watched file ops
